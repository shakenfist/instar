"""
Tests for qemu-img version detection in imago.

These tests verify that imago correctly detects the installed qemu-img version
...
class TestVersionDetection(ImagoTestBase):
    """Tests for qemu-img version detection."""

    def test_detects_qemu_version(self):
        """
        Verify imago detects the same qemu-img version as we detect in Python.

        This test compares:
        1. The version we detect by running qemu-img --version
        2. The version imago reports when run with --verbose
        """
        ...
        # Run imago with verbose output to see version detection
        imago = self.get_imago_binary()
        result = subprocess.run(
            [str(imago), '--verbose', 'info', str(image.path)],
            capture_output=True,
            text=True,
            timeout=30
        )
        ...
        if match:
            ...
            self.assertEqual(
                ...
                f'imago detected version {detected_major}.{detected_minor} '
                f'but expected {expected_major}.{expected_minor}'
            )
        else:
            # If no detection message, imago might have failed or qemu-img
            # not found - check that it still ran successfully
            self.assertEqual(
                0, result.returncode,
                f'imago failed: {result.stderr}'
            )

    def test_profile_selection_matches_version(self):
        """
        Verify imago selects the correct profile for the detected version.
        ...
        # Run imago without --qemu-version to use auto-detection
        stdout, stderr, rc = self.run_imago_info(image.path)

        self.assertEqual(0, rc, f'imago failed: {stderr}')
    ...
    def test_explicit_version_overrides_detection(self):
        ...
        stdout_v7, stderr_v7, rc_v7 = self.run_imago_info(
            image.path, qemu_version='7.2'
        )
        self.assertEqual(0, rc_v7, f'imago failed for v7.2: {stderr_v7}')

        # Run with --qemu-version 8.0 (profile-8-0-0)
        stdout_v8, stderr_v8, rc_v8 = self.run_imago_info(
            image.path, qemu_version='8.0'
        )
        self.assertEqual(0, rc_v8, f'imago failed for v8.0: {stderr_v8}')
    ...
    def test_invalid_version_string_rejected(self):
        """
        Verify imago rejects invalid --qemu-version values.
        """
        image = self.get_image('cirros-qcow2')
        if not image.path.exists():
            self.skipTest(f'Test image not found: {image.path}')
        ...
        for invalid in invalid_versions:
            ...
            result = subprocess.run(
                [str(imago), 'info', '--qemu-version', invalid, str(image.path)],
                capture_output=True,
                text=True,
                timeout=30
            )
            ...
            self.assertNotEqual(
                0, result.returncode,
                f'Expected imago to reject invalid version "{invalid}"'
            )
