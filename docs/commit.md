...
explicit `-b`. The phase 9 smoke + matrix + round-trip vmdk
...
- **Intermediate-image commit not yet supported.** When
  ...
  PLAN-rebase-commit phase 3 to be promoted to a shared
  crate.
...
master plan](plans/PLAN-rebase-commit.md):
...
- Intermediate-image commit. Needs the chain-walking
  helper from PLAN-rebase-commit phase 3 step 3f
