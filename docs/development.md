...
- `tools/ci/check-doc-phrases.sh` - Fails if any tracked file contains a plan filename mangled by a bad automated replace that spliced the word "work" into it (e.g. a sentence naming `PLAN-map` turning into one naming `PLAN-m` and `workap`); phase 9 fixed seven of these in `docs/testing.md` and a further 17 had spread before anything checked. `docs/plans/` and `tools/ci/` are exempt: the plan pages quote corrupted phrases verbatim as examples when describing the bug, and this guard's own tests build fixtures carrying the pattern on purpose
- `tools/ci/test-check-doc-phrases.sh` - Tests for that guard, including that it does not false-positive on legitimate strings like "the PLAN-map work" or "PLAN-differencing workflow"; the `ci-tooling` CI job runs it
