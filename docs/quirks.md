...
all `20240509-2+b1`, used through the `python3-libvhdi` binding — see
step 1a of
[PLAN-differencing-phase-01-pin.md](plans/PLAN-differencing-phase-01-pin.md)
for why there is no `vhdiexport` CLI). The
...
(`vhd-diff-child-aligned.vhd`, parent `vhd-diff-parent.vhd`;
`vhdx-diff-child.vhdx`, parent
...
behaviour before that commit was a reasonable "reading without a parent"
default; it was simply unimplemented composition wearing the exit code of
success.
