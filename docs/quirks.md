...
step 1a's result in that plan for why there is no `vhdiexport` CLI). The
...
Using the fixtures `scripts/create-vhd-testdata.sh` generates
(`vhd-diff-child-aligned.vhd`, parent
`vhd-diff-parent.vhd`; `vhdx-diff-child.vhdx`, parent
`.\vhdx-diff-parent.vhdx` — a different corpus from the `fat-differential`
oracle pair used above):
...
verify identity when there is one — is the shape of the fix commit
`10ab838` shipped and the composition work moves instar toward next: a typed
refusal now, and identity-checked composition later. It is quoted here as
the contrast that makes plain that neither qemu-img's behaviour nor instar's
before that commit was
