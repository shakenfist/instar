...
plans/PLAN-resize-phase-09-preallocation.md) and queued under
...
[PLAN-rebase-commit-phase-08-commit-host.md](
...
[docs/plans/PLAN-format-coverage-phase-04-qcow1-read.md](plans/PLAN-format-coverage-phase-04-qcow1-read.md)
for the full design and findings.
...
part of [PLAN-differencing.md](plans/PLAN-differencing.md). Every command
...
the read-side behaviour recorded in this section's first three subsections
was a live correctness defect, not a deliberate scope decision, when it was
...
records qemu-img's unchanged behaviour; each "instar Behavior" is split
into a "(before phase 4)" and an "(after phase 4, commit `10ab838`)" pair
recording what changed. A "Known limitations after phase 4" subsection
...
oracle); see [format-coverage.md](format-coverage.md).
...
tracked as issue #547. instar's side is fixed as of commit `10ab838`.)
...
#### instar Behavior (before phase 4)
...
`VhdState::init` received a state that looked like an ordinary dynamic disk
and had to decide for itself whether to add a differencing-specific check on
top — most did not (see the per-op table below).

`instar info` and `instar convert -O raw` on the same Hyper-V VHD child
reproduced the qemu behaviour byte for byte:
...
instar's output was bit-identical to qemu-img's mis-composed output and
`file(1)` called it unidentifiable `data`, for the same reason: sector 0
belongs to the parent, and neither reader looked at it. **instar's read
was qemu-parity, and both were silently wrong rather than right.**

#### instar Behavior (after phase 4, commit `10ab838`)

`VhdxState::init` no longer rejects `has_parent`; it reports the flag and
lets each caller decide, the way `VhdState::init` has always reported
`disk_type`. That symmetry is what let one refusal cover both formats:
`convert`, `compare` and `bench` refuse inside the shared chain
initialiser (`qcow2::init_chain_states`); `check` and `measure` refuse at
their own direct `VhdxState::init` / `VhdState::init` call sites. `info`
gained the opposite change — it now reports the parent instead of staying
silent — because it composes no sector data and has no wrong answer to
give.

Using the phase 2 fixtures (`vhd-diff-child-aligned.vhd`, parent
`vhd-diff-parent.vhd`; `vhdx-diff-child.vhdx`, parent
`.\vhdx-diff-parent.vhdx` — a different corpus from the `fat-differential`
oracle pair used above, generated for this plan's phase 2):
...
...
backing file: vhd-diff-parent.vhd
$ echo $?
0
```
...
PLAN-map.md)`, guest error code 3); its VHD wording predates this phase,
and its VHDX arm was added by this phase using the same message rather
than migrated onto the new channel, because it already worked and was
already tested. **instar is now the stricter side of this divergence,
...
**Classification: closes an Unsafe Quirk** (see above — tracked as issue
#547; fixed as of commit `10ab838`)
...
Every op in the table above now refuses both formats by name, exiting 1
with no output file left behind:
...
this phase, just for the wrong reason and with the wrong message.
...
composition wearing the exit code of success.
...
Three things surfaced while implementing phase 4 that are recorded rather
than fixed, because none of them is this phase's job to close:
...
1. **`instar info --chain` reports a one-image chain for a differencing VHD
   ...
   change which failure a user sees. Phase 14 lifts this per operation as
   composition lands. See [chain-discovery.md](chain-discovery.md#known-limitations).
