...
| Phase | Plan | Status |
|-------|------|--------|
| 1. Shared ABI: `DdConfig` struct (magic, flags, `bs`, `count`, `skip`, `output_format`, `output_cluster_bits`, precomputed `copy_len`/`out_vsize`), `DD_CONFIG_MAGIC` + flag constants in `src/shared/src/lib.rs`, host two-file (`if`/`of`) device plumbing, and the guest-binary strategy from Open question 1 | PLAN-dd-phase-01-abi.md | Not started |
