...
| 7. Cross-version baselines: add `dd` to the testdata `generate-baselines.py` + `baselines-dd` Makefile target exactly like `resize`/`amend` (create fixture → `qemu-img dd` → `qemu-img info`), curated `DD_CASES` targeting the output virtual-size rounding (512 / CHS) + empty windows; consuming test compares instar dd's result info to the qemu baseline per profile | [PLAN-dd-phase-07-baselines.md](PLAN-dd-phase-07-baselines.md) | Not started |
| 8. Coverage-guided fuzzing: fuzz the operand parser and the window/size math (`fuzz_dd_operands`, `fuzz_dd_window`) | PLAN-dd-phase-08-fuzz.md | Not started |
| 9. Differential fuzzing: add `dd` to `scripts/differential-fuzz.py` — random `bs`/`count`/`skip`/`-O` vs `qemu-img dd` | PLAN-dd-phase-09-diff-fuzz.md | Not started |
