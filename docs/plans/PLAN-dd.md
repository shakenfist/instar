...
| 5. Rust unit tests: operand parsing, size-suffix arithmetic, `count`/`skip` clamp + ordering math, output-size computation, validation/rejection paths | PLAN-dd-phase-05-rust-tests.md | Not started |
| 6. Integration tests: cross-validate the 14-row matrix against real `qemu-img dd` (`tests/test_dd.py`, `run_instar_dd` + `run_qemu_img_dd` helpers in `tests/base.py`) | PLAN-dd-phase-06-integration.md | Not started |
| 7. Cross-version baselines: capture `qemu-img dd` reference outputs in the testdata repo across qemu versions | PLAN-dd-phase-07-baselines.md | Not started |
