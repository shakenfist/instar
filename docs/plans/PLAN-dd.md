...
| 3. Guest op — raw output: make `convert_to_raw` honour `FLAG_DD_WINDOW` (loop `[window_start, window_end)`, re-base output addressing — unaligned `window_start` works since `read_chain_virtual_cluster` reads arbitrary offsets), add `has_dd_window()` accessor, and fix the host to truncate dense raw output to `out_vsize` (the existing truncation is `skip_zeros`-gated). Short final block + empty-output (count=0 / skip-past-EOF) cases; raw window matrix cross-validated vs `qemu-img dd`. Non-raw formats deferred to phase 4 | [PLAN-dd-phase-03-guest-raw.md](PLAN-dd-phase-03-guest-raw.md) | Not started |
