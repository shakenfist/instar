...
Master plan: [PLAN-measure.md](PLAN-measure.md) ·
Previous phase: [PLAN-measure-phase-09-fuzz-differential.md](PLAN-measure-phase-09-fuzz-differential.md)
...
```
- [Measure](measure.md) — predict file size for a target format
```
...
| 10c | low | sonnet | none | Update `ARCHITECTURE.md`'s Format Support section with the "Measurable target formats" line. Update `AGENTS.md`'s operations list to add measure with a one-sentence pointer. Update `docs/plans/PLAN-convert-followups.md` to strike measure from the seven-subcommand deferred list (replace with `~~measure~~ — shipped, see [PLAN-measure.md](PLAN-measure.md)` or similar; preserve the strike-through). Run pre-commit. Touch only those three files. |
