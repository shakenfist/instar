...
plan is [PLAN-rebase-commit.md](PLAN-rebase-commit.md). This
...
|------|--------|-------|-----------|---------------------|
| 3a | medium | sonnet | none | Extend `RebaseResult` in `src/shared/src/lib.rs`. Append the 7 new error code constants from open question 4 (`ERROR_OVERLAY_CORRUPT = 7` through `ERROR_INTERNAL_OVERFLOW = 13`) inside the existing `impl RebaseResult` block. Update the doc comments on `RebaseError` in `src/crates/rebase/src/lib.rs` to cross-reference the new wire codes. Add a unit test to the existing `mod tests` block in `src/shared/src/lib.rs` asserting the new constants are distinct from the old ones. No behavioural changes. |
| 3b | medium | sonnet | none | Scaffold `src/operations/rebase/`: `Cargo.toml` (mirror `src/operations/resize/Cargo.toml`, dependencies on `shared`, `rebase` planner crate, `qcow2`, `vmdk`; `[profile.release]` with `panic = "abort"`, `opt-level = "z"`, `lto = true`; binary name `rebase`); `src/main.rs` with the `_start` skeleton, the `validate_call_table!` macro use, config + header reads, format dispatch stubs that return `ERROR_UNSUPPORTED_FORMAT`, and the result-send call. Add the crate to the workspace members list in `src/Cargo.toml`. Add the binary to `scripts/check-binary-sizes.sh` if it has a hardcoded list. `make instar` builds clean. |
| 3c | high | opus | none | Implement the qcow2 unsafe-mode path in `src/operations/rebase/src/main.rs`. Add `run_qcow2_unsafe(call_table, config) -> RebaseResult`. The function: reads the overlay header into `HEADER_BUF` and parses via `qcow2::QcowHeader::parse`; populates `Qcow2RebaseOpts` from the parsed header and `config.new_backing_path[..config.new_backing_path_len]`; calls `plan_rebase_qcow2`; dispatches the resulting `Unsafe { plan }` through an `apply_plan` helper modelled on `src/operations/resize/src/main.rs:273` `apply_plan`; returns the result. Add the `apply_plan` helper and the supporting `write_byte_range` (copy from resize lines 180–221) and `read_byte_range` helpers. Add the `map_rebase_error` helper that translates every `RebaseError` variant to its wire code. Use opus: the qcow2 header offset math and the byte-aligned write loop need careful correctness. |
| 3d | medium | sonnet | none | Implement the vmdk unsafe-mode path in `src/operations/rebase/src/main.rs`. Add `run_vmdk_unsafe(call_table, config) -> RebaseResult`. The function: reads the overlay header, parses via `vmdk::Vmdk4HeaderFull::parse`; reads the descriptor region into `EXISTING_STATE` scratch via `read_byte_range`; reads the new backing's descriptor via `read_input_sector(config.new_chain_first, ...)` and parses it via `vmdk::parse_descriptor` to extract the parent CID (default to 0xffffffff if extraction fails); populates `VmdkRebaseOpts` with the staged descriptor bytes, the slot size from the header, and the parent CID; calls `plan_rebase_vmdk`; applies the resulting single-patch plan. Reuses `apply_plan` from step 3c. |
| 3e | high | opus | none | Implement the qcow2 safe-mode path in `src/operations/rebase/src/main.rs`. Add `run_qcow2_safe(call_table, config) -> RebaseResult`. The function: (1) reads the overlay header, parses it, reads the full L1 table and the entire L2 region into scratch (open question 2, choice C); (2) reads the refcount table and all refcount blocks it points at into scratch alongside an array of `refblock_host_offsets`; (3) populates `Qcow2RebaseOpts` with all the safe-mode fields and calls `plan_rebase_qcow2`; (4) extracts `Qcow2RebaseOutput::Safe { context, deferred_metadata }`; (5) drives the comparison loop (see section below); (6) flushes dirty refcount blocks via `write_byte_range`; (7) applies `deferred_metadata` via `apply_plan`; (8) returns the result with `mode = MODE_SAFE` and `clusters_copied = state.allocated`. The comparison loop iterates `0..context.overlay_cluster_count` and for each cluster: looks up the L2 entry from the staged L2 region; if allocated, skips; else calls a `read_chain_cluster` helper to read both old and new chains at the guest offset; compares; if differ, calls `allocate_overlay_cluster_qcow2`, writes the old-chain bytes to the claimed cluster offset, and updates the corresponding L2 entry in the staged L2 region. The L2 region must also be flushed after the loop (alongside refcount blocks) before the deferred metadata applies. Use opus: this is the trickiest correctness work in phase 3 — L1/L2 decode, chain walk, allocator threading, ordering. |
| 3f | medium | sonnet | none | Add the `read_chain_cluster` helper to `src/operations/rebase/src/main.rs` (or a sibling module). Signature: `unsafe fn read_chain_cluster(call_table: &CallTable, first_device_idx: u32, device_count: u32, guest_offset: u64, cluster_size: u32, out_buf: *mut u8) -> bool`. Implementation: iterates devices from `first_device_idx`, for each device parses its header to determine cluster ownership for the given guest offset (qcow2 walks L1/L2; vmdk walks GD/GT; raw is byte-addressable), returns true if a device covers the offset and was successfully read, false otherwise. v1 supports qcow2 and raw chain members only; vmdk in the chain returns false (the host validates the chain is all-qcow2 before launching). Document this v1 limitation in the helper's doc-comment. |
| 3g | low | sonnet | none | Run `pre-commit run --all-files`. Resolve rustfmt / clippy findings. Run `make instar`, `make check-binary-sizes`. Verify the new binary loads cleanly and is well under 384 KB. Update the execution table row for phase 3 in `docs/plans/PLAN-rebase-commit.md` to mark each shipping commit. Update the master plan's Future Work to record vmdk safe-mode rebase deferral (depends on phase 2 step 2e). |
