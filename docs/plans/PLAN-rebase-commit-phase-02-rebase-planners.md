...
plan is [PLAN-rebase-commit.md](PLAN-rebase-commit.md). Phase
1 ([PLAN-rebase-commit-phase-01-abi.md](PLAN-rebase-commit-phase-01-abi.md))
...
|------|--------|-------|-----------|---------------------|
| 2a | medium | sonnet | none | Scaffold `src/crates/rebase/`: `Cargo.toml` declaring no-std with path-deps on `shared`, `qcow2`, `vmdk`, `create`; `src/lib.rs` with crate-level `#![no_std]`, `#![allow(clippy::too_many_arguments)]`, the public types (`RebasePatch<'a>`, `RebasePlan<'a>` with `MAX_REBASE_PATCHES = 64`, `RebaseError` enum, `RebaseMode { Unsafe, Safe }`), and stub entry points `plan_rebase_qcow2` and `plan_rebase_vmdk` that return `RebaseError::UnsupportedFormat`. Add the crate to the workspace members list in `src/Cargo.toml`. Mirror the `src/crates/resize/Cargo.toml` layout exactly. After this step `make instar` builds clean and clippy is silent. |
| 2b | high | opus | none | Implement qcow2 unsafe-mode planner in `src/crates/rebase/src/qcow2.rs` (new private module). Public-to-crate function `plan_qcow2_unsafe(opts, scratch) -> Result<RebasePlan, RebaseError>`. Computes: (1) validation — refuse external-data-file qcow2 with `ERROR_EXTERNAL_DATA_FILE`, refuse if `new_backing_path_len > backing_file_size_max` (1024), refuse on `OverlayCorrupt` if `QcowHeader::parse` returns None; (2) header rewrite bytes — `backing_file_offset` (u64 BE) and `backing_file_size` (u32 BE) at header offsets 8 and 16; (3) the path string at the existing `backing_file_offset` slot if `new_len <= old_size`, else allocate via the qcow2 allocator from step 2c (which is the same allocator used in safe mode) and write at the new offset; (4) optional backing-format extension — see open question 4. Emit patches into the `RebasePlan` in the order the guest must apply them: refcount + L2 + path string first, header field rewrite last. Use opus: this step needs to hold the qcow2 header layout, the backing-extension parser at `src/crates/qcow2/src/lib.rs:470`, the create-side header-write logic in `src/crates/create/src/lib.rs`, and the allocator interface in mind at the same time. Note the dependency: step 2b depends on the allocator from step 2c being available; do 2c first or stub the allocator until 2c lands. |
| 2c | high | opus | none | Implement qcow2 safe-mode planner + allocator in `src/crates/rebase/src/qcow2.rs`. Add: (1) `RebaseQcow2SafeContext<'a>` struct carrying the fields from open question 6, plus a scratch-slice view of the refcount-block bytes; (2) `AllocationState` struct (next-cluster-to-scan offset, refcount-blocks-mutated bitmap, allocation counter); (3) `plan_qcow2_safe(opts, scratch) -> Result<(RebaseQcow2SafeContext, RebasePatch), RebaseError>` that validates compatibility (overlay.virtual_size <= new_backing.virtual_size; both qcow2; new backing not LUKS), pre-loads the refcount-block bytes into scratch via the input slices `opts` provides, computes the header rewrite patch (deferred-apply), and returns the context; (4) `allocate_overlay_cluster_qcow2(ctx, state) -> Result<u64, RebaseError>` — pure function. Scans `ctx.refcount_blocks` for the next entry whose refcount is 0 starting from `state.next_scan_offset`, bumps that entry's refcount to 1 (using `lookup_refcount`-style decoding for the configured `refcount_bits`), and returns the host-byte cluster offset. Returns `RebaseError::RefcountExhausted` when all blocks are full. Update `state.next_scan_offset` so subsequent calls don't repeat. The function does NO I/O; the guest is responsible for writing the dirty refcount block back when the loop completes. Use opus: refcount-width handling (1/2/4/8/16/32/64 bits) is the trickiest correctness work in this phase. Look at `src/crates/qcow2/src/lib.rs:2980` `lookup_refcount` as the reference for the bit-packing semantics. |
| 2d | medium | sonnet | none | Implement vmdk unsafe-mode planner in `src/crates/rebase/src/vmdk.rs` (new private module). Public-to-crate function `plan_vmdk_unsafe(opts, scratch) -> Result<RebasePlan, RebaseError>`. Validates subformat == monolithicSparse (reject others with `UnsupportedSubformat` — append the variant to `RebaseError` if not already present from step 2a). Computes new descriptor text by reusing `create::build_vmdk_descriptor_with_backing` from `src/crates/create/src/lib.rs` if the helper is public; otherwise add a `pub fn` wrapper around it. Emits a single Write patch over the descriptor region at `header.desc_offset_sectors * 512`, length `header.desc_size_sectors * 512`, padded with NUL bytes to fill the slot. Refuse if the new descriptor exceeds the slot (return `RebaseError::DescriptorTooLarge` — new variant). The new descriptor must preserve the existing `CID=` value but update `parentCID=` (to the new backing's CID from `opts.new_parent_cid`) and `parentFileNameHint=` (to the new backing path). The existing descriptor's createType and extent lines must also be preserved. Spend time reading `src/crates/vmdk/src/lib.rs:1114` `parse_descriptor` to understand exactly what fields are extracted and what must be passed back through. |
| 2e | high | opus | none | Implement vmdk safe-mode planner + grain allocator in `src/crates/rebase/src/vmdk.rs`. Add: `RebaseVmdkSafeContext<'a>` with grain-directory layout fields; `allocate_overlay_grain_vmdk(ctx, state) -> Result<u64, RebaseError>` that scans the grain directory for an unallocated grain table (GDE == 0) and within an allocated grain table for an unallocated grain (GTE == 0), and returns the host sector offset of a freshly claimed grain. Mirror the qcow2 allocator's semantics but adapted to vmdk's two-level grain table. The descriptor rewrite patch is deferred-apply, same shape as 2c. Use opus: the grain-table layout (grain size in sectors, 128 GTEs per grain table, 512 bytes per grain table) needs to be held in mind alongside the descriptor format. If the existing vmdk crate doesn't expose enough primitives, add them as `pub` to `src/crates/vmdk/src/lib.rs` rather than reimplementing in rebase. |
| 2f | medium | sonnet | none | Integration tests under `src/crates/rebase/tests/`. Create three test files: `qcow2_unsafe.rs`, `qcow2_safe.rs`, `vmdk_unsafe.rs`. Each follows the `src/crates/resize/tests/qcow2_grow.rs` pattern: build a source image via `create::plan_*`, materialize bytes into a `Vec<u8>`, build `Qcow2RebaseOpts` / `VmdkRebaseOpts`, call `plan_rebase_*`, apply the patches, re-parse, assert metadata. Required test cases per the Mission section: qcow2 unsafe basic rebase, qcow2 unsafe with long path forcing relocation, qcow2 safe-mode smoke (assert context's claims), qcow2 safe-mode allocator unit test (hand-crafted refcount block with two free entries), vmdk unsafe basic rebase. Error-path tests: incompatible virtual sizes, oversized backing path, external data file. Do not exhaustively cross-validate against qemu-img in this phase — that's phase 5's job. |
| 2g | low | sonnet | none | Run `pre-commit run --all-files`. Resolve any rustfmt / clippy findings. Run `make instar`, `make test-rust`, `make check-binary-sizes`. Verify the new crate is reachable from the workspace and that the resize crate did not regress. Update the execution table row for phase 2 in `docs/plans/PLAN-rebase-commit.md` to `Complete (<commit-hash>)`. Stage and present the commit(s) — one per step if review prefers, or one consolidated commit, with the commit-message convention from CLAUDE.md (50-char first line, 75-char body wrap, Prompt paragraph, Signed-off-by, Co-Authored-By with model + context window + effort level). |
