...
| 2e | high | opus | none | **Deferred to follow-up.** Implement vmdk safe-mode planner + grain allocator in `src/crates/rebase/src/vmdk.rs`. Add: `RebaseVmdkSafeContext<'a>` with grain-directory layout fields; `allocate_overlay_grain_vmdk(ctx, state) -> Result<u64, RebaseError>` that scans the grain directory for an unallocated grain table (GDE == 0) and within an allocated grain table for an unallocated grain (GTE == 0), and returns the host sector offset of a freshly claimed grain. Mirror the qcow2 allocator's semantics but adapted to vmdk's two-level grain table. The descriptor rewrite patch is deferred-apply, same shape as 2c. Use opus: the grain-table layout (grain size in sectors, 128 GTEs per grain table, 512 bytes per grain table) needs to be held in mind alongside the descriptor format. If the existing vmdk crate doesn't expose enough primitives, add them as `pub` to `src/crates/vmdk/src/lib.rs` rather than reimplementing in rebase. **Note:** vmdk backing-chain rebase has a smaller user base than qcow2; deferring until phase 3 (rebase guest binary) is wired so we can prioritise based on demand. |
| 2f | medium | sonnet | none | **Deferred to follow-up.** Cross-format integration tests under `src/crates/rebase/tests/`. Create three test files: `qcow2_unsafe.rs`, `qcow2_safe.rs`, `vmdk_unsafe.rs`. Each follows the `src/crates/resize/tests/qcow2_grow.rs` pattern: build a source image via `create::plan_*`, materialize bytes into a `Vec<u8>`, build `Qcow2RebaseOpts` / `VmdkRebaseOpts`, call `plan_rebase_*`, apply the patches, re-parse, assert metadata. Required test cases per the Mission section: qcow2 unsafe basic rebase, qcow2 unsafe with long path forcing relocation (will currently fail — relocation is deferred), qcow2 safe-mode smoke (assert context's claims), qcow2 safe-mode allocator unit test (hand-crafted refcount block with two free entries), vmdk unsafe basic rebase. **Note:** the in-crate unit tests already cover most of the assertions a phase 2 integration test would make; full cross-format integration belongs together with phase 3 (the rebase guest binary) when there is something to drive end-to-end. |
| 2g | low | sonnet | none | **Partial.** Run `pre-commit run --all-files`. Resolve any rustfmt / clippy findings. Verify the new crate is reachable from the workspace and that the resize crate did not regress. Update the execution table row for phase 2 in `docs/plans/PLAN-rebase-commit.md` to mark partial completion with the shipping commit hashes. Document the deferrals (2e, 2f, long-path relocation, refcount widths other than 16, backing-format extension writer) in this phase plan and in the master plan's Future Work section. |
