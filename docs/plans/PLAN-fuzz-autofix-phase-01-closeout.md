...
* **`coverage-fuzz.yml` is timing out, and it costs the corpus —
  ...
  [PLAN-coverage-fuzzing.md](PLAN-coverage-fuzzing.md), not here.
...
3. `grep -c 'output-format text' .github/workflows/fuzz-autofix.yml
   ...
   tools/address-comments-with-claude.sh` is 0 for all three, and each
   of the three passes `--output-format stream-json --verbose`.
