...
|------|--------|-------|-----------|-------|
| 5a | low | (agent, on operator approval) | none | Create the "Develop branch" ruleset via `POST /repos/shakenfist/instar/rulesets` with the body recorded in `docs/development.md`, then read it back and diff against intent. Reversible with `DELETE`. Must come **after** step 2 of the ordering above. |
