...
[PLAN-bitmap.md](PLAN-bitmap.md); Phases 1–9 are complete —
...
- **`docs/usage.md`**: the platform matrices already list `bitmap`
  ...
  **no edit needed there**. The `#### bitmap operations` block
  ...
  block + one prose paragraph + a `See [docs/bitmap.md](bitmap.md)`
  line).
