...
| 2. qemu-img version→profile coverage + live version-detection | PLAN-distro-matrix-ci-phase-02-qemu-profiles.md | Code complete. Matrix qemu versions enumerated (tools/probe-qemu-versions.sh); full-version profile/baseline selection replaces the 7.2.19-boundary prefix bug (only Debian 12 mis-selected, and benignly — differing fields are normalised); parsers pinned both sides; docs/testing.md updated. NB the "no version.rs widen needed" finding was INFO-only and is superseded by phase 2b (map/snapshot do need widening). |
| 2b. qemu output-parity widen (map/snapshot on older qemu) | PLAN-distro-matrix-ci-phase-02b-qemu-output-parity.md | Planned. Phase-3 full runs found instar emits newest-qemu output for map (compressed field, added at qemu 8.2) and snapshot -l (header format, changed ~qemu 9.0) regardless of the emulated version, so it diverges on every pre-8.2 distro (Debian 12, Ubuntu 22.04: 19 map + 1 snapshot failures each). Both fixes are host-side version gates. map boundary is empirically solid; snapshot needs an independent boundary + a snapshot-list-human testdata baseline fix; one qcow1→vpc geometry case is TBD. Phase 4 depends on this. |
