...
## Status: Phases 1–3 and 2b complete; phases 4–5 drafted
...
| 4. Workflow integration (merge_group package-matrix job) | PLAN-distro-matrix-ci-phase-04-workflow.md | **Planned in detail 2026-08-11**, not yet implemented (4a-4g). The plan is grounded in the sibling repo's merge-queue implementation and corrects the original sketch, whose step 4e would have broken PR CI: this repo has no aggregate gate today, and the `needs:` list the sketch pointed at belongs to `automated_reviewer`. Phase 4 creates a `can_merge` gate instead. Open operator questions: the `xl` runner pool size (sets `max-parallel`) and whether `GITLAB_TESTDATA_TOKEN` reaches `merge_group`. |
