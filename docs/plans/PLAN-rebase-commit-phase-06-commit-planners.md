...
is [PLAN-rebase-commit.md](PLAN-rebase-commit.md). Phases 1–5
...
|------|--------|-------|-----------|---------------------|
| 6a | medium | sonnet | none | Scaffold `src/crates/commit/`: `Cargo.toml` declaring `no_std` with path-deps on `shared`, `qcow2`, `vmdk`, `create`; `src/lib.rs` with crate-level `#![no_std]`, `#![allow(clippy::too_many_arguments)]`, the public types (`CommitPatch<'a>`, `CommitPlan<'a>` with `MAX_COMMIT_PATCHES = 16`, `CommitError` enum, `BackingAllocationState`, `BackingGrainAllocationState`), and stub entry points `plan_commit_qcow2` and `plan_commit_vmdk` that return `CommitError::UnsupportedFormat`. Add the crate to the workspace members list in `src/Cargo.toml`. Add `make test-rust` exclusions for the new crate (none expected — unit tests run normally). Mirror the `src/crates/rebase/Cargo.toml` layout exactly. After this step `make instar` builds clean and clippy is silent. |
| 6b | high | opus | none | Implement the qcow2 commit planner + backing allocator in `src/crates/commit/src/qcow2.rs` (new private module). Adds `Qcow2CommitOpts<'a>`, `Qcow2CommitContext<'a>`, `plan_commit_qcow2`, `allocate_backing_cluster_qcow2`, `overlay_l2_byte_offset`, and `overlay_refcount_byte_offset_qcow2`. Validation: parse both overlay and backing headers via `QcowHeader::parse` and refuse on `OverlayCorrupt` / `BackingCorrupt`; refuse `INCOMPAT_EXTERNAL_DATA` on either side; refuse `crypt_method != 0`; refuse `backing.virtual_size < overlay.virtual_size`; v1 supports `refcount_bits == 16` only on the backing (matches rebase phase 2 step 2c). Allocator is a near-copy of `rebase::allocate_overlay_cluster_qcow2` retargeted at the backing's refcount blocks; the planner stages those into scratch the same way step 2c does. Unit tests: at least one per validation arm plus the two allocator tests (claim-and-advance + exhausted). **Caveat:** "skip when chain provides" mode is deferred (see open question 3). |
| 6c | high | opus | none | Implement the vmdk commit planner + backing grain allocator in `src/crates/commit/src/vmdk.rs` (new private module). Adds `VmdkCommitOpts<'a>`, `VmdkCommitContext<'a>`, `plan_commit_vmdk`, `allocate_backing_grain_vmdk`, and `overlay_gte_byte_offset_vmdk`. The grain allocator is structurally identical to rebase step 2e's `allocate_overlay_grain_vmdk`: bump a `next_grain_sector` cursor by `backing_grain_size_sectors`, return the host byte offset. Validation: parse both headers via `Vmdk4HeaderFull::parse`, refuse vmdk subformats other than `monolithicSparse`, refuse on backing smaller than overlay. v1 only allocates into GTs the backing already has — the GD-extension case (allocating a fresh GT when the covering GDE is zero on the backing) is a follow-up tracked under "Future work" (mirrors rebase phase 2 step 2e). Unit tests: smoke (planner returns sane geometry) + allocator (lands at backing EOF, advances by grain). |
| 6d | medium | sonnet | none | Cross-format integration tests under `src/crates/commit/tests/`. Three test files plus a shared `tests/common/mod.rs` helper (the rebase crate's `tests/common/mod.rs` pattern is the template): `qcow2.rs` (smoke: plan commit against a backing+overlay built via `create::plan_qcow2`, drive the allocator twice, assert host offsets land at fresh cluster boundaries, assert refblock bytes are bumped; plus error paths for external data file / LUKS / backing-smaller-than-overlay), `vmdk.rs` (smoke + allocator lands-at-EOF, mirroring `tests/vmdk_safe.rs` from the rebase crate), and `common/mod.rs` exposing `materialise_create` (copied from rebase) so the tests can turn a `MetadataPlan` into `Vec<u8>`. The tests do not exercise the per-cluster commit loop — that's a phase 9 integration test against the host binary. |
| 6e | low | sonnet | none | Run `pre-commit run --all-files`. Verify the new crate compiles in isolation (`cargo build -p commit` via the lint container). Verify `make check-binary-sizes` is unchanged (the crate is library-only; nothing new ships in a binary until phase 7). Update the execution-table row for phase 6 in `docs/plans/PLAN-rebase-commit.md` to mark each shipping commit. Document anything that surfaced during 6b–6d in this phase plan's "Future work created by this phase" section. |
