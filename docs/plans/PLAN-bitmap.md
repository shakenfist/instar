...
| 5. Host VMM subcommand: `BitmapArgs` clap surface (ordered repeatable action flags, `-g`/`-b`/`-F`/`-f`, `--object`/`--image-opts` refusal, `--output`), argument-ordering validation matching qemu, host probe, `run_bitmap`/`run_bitmap_guest`, (same-image and, if in scope, cross-file) source wiring, silent/json render, error mapping (OQ2, OQ11) | PLAN-bitmap-phase-05-host-cli.md | Not started |
| 6. Rust round-trip tests (`src/crates/bitmap/tests/`): per-action mutate → re-parse, assert directory/table/refcount invariants and empty-bitmap representation | PLAN-bitmap-phase-06-rust-tests.md | Not started |
| 7. Python integration tests (`tests/test_bitmap.py`): cross-check vs `qemu-img bitmap` with post-op `qemu-img check`/`info`, error paths (v2 reject, name exists, not found, granularity range, non-qcow2 reject, in_use rules), known-divergence registry | PLAN-bitmap-phase-07-integration.md | Not started |
| 8. Cross-version baselines: `BITMAP_CASES` in `generate-baselines.py`, `expected-outputs/`, testdata push | PLAN-bitmap-phase-08-baselines.md | Not started |
| 9. Coverage fuzz (`fuzz_bitmap_*` for the directory/table parser and the planner) + differential fuzz (`op_bitmap` in `differential-fuzz.py` vs `qemu-img bitmap`) | PLAN-bitmap-phase-09-fuzz.md | Not started |
| 10. Docs: `docs/bitmap.md`, `docs/usage.md`, `CHANGELOG.md`, `ARCHITECTURE.md`/`README.md`/`AGENTS.md`, `docs/plans/index.md`/`order.yml` | PLAN-bitmap-phase-10-docs.md | Not started |
