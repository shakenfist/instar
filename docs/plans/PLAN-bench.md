...
| 7. Fuzz: `fuzz_bench_schedule` coverage target (schedule/validation invariants) registered in the nightly tiers; differential `op_bench` in `scripts/differential-fuzz.py` over the deterministic surface only (OQ13) | [PLAN-bench-phase-07-fuzz.md](PLAN-bench-phase-07-fuzz.md) | Planned (steps 7a-7c; fast-tier coverage target re-implementing the crate's test invariants per house convention; op_bench with a steer-around picker avoiding all 11 registry divergences; oracles are exit codes, header parity, and post-write image state — never durations) |
| 8. Docs (closes the plan): `docs/bench.md` user guide — **what the number means** (sandboxed-path measurement, sandbox-overhead methodology vs `qemu-img bench`, serialized-depth caveat), divergence table, destructive-`-w` warning; `docs/usage.md` section + parity-table row update; `ARCHITECTURE.md`/`AGENTS.md`/`README.md`/`CHANGELOG.md`; plans index status → Complete | PLAN-bench-phase-08-docs.md | Not started |
