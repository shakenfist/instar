...
| 5d | high | opus | none | **Shipped** in `instar-testdata` (`f24b1bc37` script + `ab587c0cd` baselines). Extended `generate-baselines.py` with `REBASE_CASES` + `generate_rebase_baseline` + `--command rebase` dispatcher arm. Generated 480 baselines across 80 qemu-img versions × 6 cases (qcow2 only — qemu-img rebase rejects every other format with "Operation not supported"). Cases: 4 unsafe (`-u`) shapes plus 2 safe shapes (same-size, detach). |
