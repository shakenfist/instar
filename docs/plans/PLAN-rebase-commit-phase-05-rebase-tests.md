...
| 5d | high | opus | none | **Deferred to follow-up.** Extend instar-testdata/scripts/generate-baselines.py to support --command rebase. Add REBASE_CASES dict and generate_rebase_baseline helper. Lives in a separate repo (instar-testdata); committed there separately from this branch. |
| 5e | high | opus | none | **Deferred to follow-up alongside 5d.** TestRebaseBaselineMatrix mirroring TestCreateBaselineMatrix in tests/test_create.py:817-1012 — factory-generated per-(target, case) tests against the baselines from step 5d. |
| 5f | medium | sonnet | none | **Deferred to follow-up alongside phase 4 step 4d.** Round-trip helper test using assert_info_equivalent from tests/helpers/info_json.py. Depends on 4d landing first so the rebase guest can actually succeed. |
| 5g | low | sonnet | none | **Partial.** Pre-commit clean. Master plan updated to reflect partial completion with shipping commit hashes. |
