...
|------|--------|-------|-----------|---------------------|
| 7a | medium | sonnet | none | Extend `CommitResult` in `src/shared/src/lib.rs`. Append the 6 new error code constants for `CommitError` variants that don't yet have a wire equivalent: `ERROR_OVERLAY_CORRUPT = 8`, `ERROR_BACKING_CORRUPT = 9`, `ERROR_SCRATCH_TOO_SMALL = 10`, `ERROR_REFCOUNT_EXHAUSTED = 11`, `ERROR_PARSE_FAILED = 12`, `ERROR_INTERNAL_OVERFLOW = 13`. Update the doc comments on `CommitError` in `src/crates/commit/src/lib.rs` to cross-reference the new wire codes. Add a unit test to the existing `mod tests` block in `src/shared/src/lib.rs` asserting the new constants are distinct from the old ones. No behavioural changes. |
| 7b | medium | sonnet | none | Scaffold `src/operations/commit/`: `Cargo.toml` mirroring `src/operations/rebase/Cargo.toml` (no_std, deps on shared / commit / qcow2 / vmdk); `.cargo/config.toml` and `linker.ld` copied from rebase; `src/main.rs` with `_start` + scratch layout + stub `run_qcow2` / `run_vmdk` returning `ERROR_UNSUPPORTED_FORMAT`. Add `commit` to workspace members in `src/Cargo.toml`. Update `src/build.sh` build steps, `scripts/check-binary-sizes.sh` op list (target name `commit.bin`), `scripts/check-rust.sh` clippy exclusion list, `Makefile` `CARGO_TOML_FILES` + `test-rust` exclusions. After this step `make instar` builds `commit.bin` clean and `make check-binary-sizes` reports the binary's size. |
| 7c | high | opus | none | Implement `run_qcow2` in `src/operations/commit/src/main.rs`. Stages overlay L1 + L2 tables + backing L1 + L2 tables + refcount table + refblock host offsets + refcount blocks (the rebase phase 3 step 3e shape, duplicated for the backing side). Calls `plan_commit_qcow2`, initialises `BackingAllocationState`, then runs the per-cluster commit loop documented above. Handles the "backing has no L2 table at this L1 idx" case by allocating a fresh L2 via the same allocator (open question 3 option A). Refuses compressed L2 entries with `ERROR_UNSUPPORTED_FORMAT`. Refuses cluster-size mismatches with `ERROR_UNSUPPORTED_FORMAT` (open question 2 v1 deferral). Performs the batched overlay-clear pass via `write_input_sector(0, ...)`. Flushes dirty backing L2 / L1 / refcount blocks via `write_output_sector`. Re-reads the backing header for the `HeaderMismatch` check. Adds `map_commit_error` exhaustive mapping. Binary size budget: aim for under 100 KB; if the L2-extension case pushes past it, fall back to open question 3 option B and document the deferral. |
| 7d | medium | sonnet | none | Implement `run_vmdk` in `src/operations/commit/src/main.rs`. Stages overlay GD + GTs + backing GD + GTs. Calls `plan_commit_vmdk`, initialises the EOF allocator via `BackingGrainAllocationState::at_eof(backing_file_size, backing_grain_size_sectors)`. Runs the per-grain commit loop, handling the "backing has no GT at this GD idx" case by skipping the grain (v1 deferral — phase 6 step 6c's caveat). Performs the overlay-clear pass: write zero u32 GTEs via `write_input_sector(0, ...)`. Flushes dirty backing GTs via `write_output_sector`. No backing refcount blocks (vmdk has none). Re-reads the backing header for the `HeaderMismatch` check. |
| 7e | low | sonnet | none | Run `pre-commit run --all-files`. Verify the new binary builds clean (`commit.bin` under 384 KB; aim for under 100 KB). Update the execution-table row for phase 7 in `docs/plans/PLAN-rebase-commit.md` to mark each shipping commit. Document anything that surfaced during 7c–7d in this phase plan's "Future work created by this phase" section. |
