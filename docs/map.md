...
Each entry is documented in [quirks.md](quirks.md) with the
...
- **Raw source sparseness is not detected** — instar reports raw
  ...
  See [docs/quirks.md § Raw source sparseness is not detected](quirks.md#raw-source-sparseness-is-not-detected).
- **VHD unallocated blocks are reported as `present: false`** —
  ...
  [docs/quirks.md § VHD unallocated blocks are reported as `present: false`](quirks.md#vhd-unallocated-blocks-are-reported-as-present-false).
- **VHDX `PARTIALLY_PRESENT` blocks are reported as `data: true`**
  ...
  [docs/quirks.md § VHDX `PAYLOAD_BLOCK_PARTIALLY_PRESENT` is reported as `data: true`](quirks.md#vhdx-payload_block_partially_present-is-reported-as-data-true).
...
- **qcow2 compressed clusters report `compressed: false`** —
  ...
  stripped. See
  [docs/quirks.md § qcow2 compressed clusters report `compressed: false`](quirks.md#qcow2-compressed-clusters-report-compressed-false).
- **Backing-chain `depth` is always 0 in v1** — instar refuses
  sources with a backing pointer; qemu-img walks the chain. See
  [docs/quirks.md § Backing-chain `depth` is always 0 in v1](quirks.md#backing-chain-depth-is-always-0-in-v1).
- **`--image-opts` is rejected** — qemu-img's descriptor-based
  ...
  See [docs/quirks.md § `--image-opts` is rejected](quirks.md#--image-opts-is-rejected).
- **Window filter is byte-level, not cluster-aligned** —
  ...
  [docs/quirks.md § Window filter is byte-level, not cluster-aligned](quirks.md#window-filter-is-byte-level-not-cluster-aligned).
