...
## Status: Phase 0 Complete - Multi-Device VMM & Chain Discovery
...
- All operations (info, copy) now use DeviceSet for device management
...
- `CHAIN_CONFIG_ADDR` (0x1A000) for passing chain metadata to guest
- New call table function `get_chain_config()` (VERSION bumped to 9)
- VMM helper functions to convert `BackingChain` to guest-compatible `ChainConfig`
...
- 44 Rust unit tests (including chain.rs path resolution, security, chain config)
- 301+ Python integration tests verifying output against qemu-img baselines

**Next Phase:**
- Phase 1: `imago check` - Validate image structural integrity
...
6. **Phase 4: `imago convert` raw→qcow2** - Write valid qcow2 output
...
## Phase 4: `imago convert` raw→qcow2

**Goal**: Create valid qcow2 from raw input.
