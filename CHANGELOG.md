...
- **Host contention in the merge queue no longer ejects green pull
  ...
  reference `qemu-img commit` it compares instar against through a
  `_run_tool` helper whose default timeout was 60 seconds, while every
  sibling qemu call in the same test allowed 120. The distro matrix
  runs only in the merge queue and fans several
  4-worker suites out onto one self-hosted runner, so roughly twenty
  qemu processes compete there; under that load the reference commit
  ran past 60 seconds and the run failed with a `TimeoutExpired` on
  stock `qemu-img`, with instar not in the code path at all. Six legs
  across four distros and four qemu major versions were ejected this
  way, every one of them for a pull request whose diff went nowhere
  near the commit operation
  ([#528](https://github.com/shakenfist/instar/issues/528)). The helper
  was copy-pasted into five test classes across three modules, each
  carrying the same arbitrary budget; it now lives once on
  `InstarTestBase` with a `QEMU_TOOL_TIMEOUT` of 300 seconds, which is
