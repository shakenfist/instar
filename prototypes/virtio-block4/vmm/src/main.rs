//! Minimal KVM VMM with virtio-block device support and performance statistics.
//!
...
//!
//! Both devices support configurable sector sizes via command-line options.
//! Configuration is sent to the guest over the serial port at startup using
//! a protobuf message.
//!
...
//!
//! This prototype adds comprehensive performance statistics tracking:
//! - VM exit counts by type (I/O, MMIO, HLT, etc.)
...
// Stack needs to be large enough for the compiler's stack probing (192KB+)
// Place it at 3MB with 1MB size to leave room for virtqueues and DMA pool
const STACK_BASE: u64 = 0x300000; // 3MB
const STACK_SIZE: u64 = 0x100000; // 1MB
...
#[derive(Parser, Debug)]
#[command(name = "vmm")]
#[command(about = "Virtio-block VMM with performance statistics")]
struct Args {
    ...
    /// Sector size for input device in bytes (default: 512)
    #[arg(long, default_value = "512")]
    input_sector_size: u32,

    /// Sector size for output device in bytes (default: 512)
    #[arg(long, default_value = "512")]
    output_sector_size: u32,

    /// Progress update interval as percentage (0=every 10 sectors, 1-99=every N%, 100=none)
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "Output file: {} (pre-allocated {} bytes, {} sectors @ {} bytes/sector)",
        args.output,
        input_size,
        input_size / args.output_sector_size as u64,
    ...
    let mut vmm_stats = VmmStats::new();
    ...
    loop {
        match vcpu.run()? {
            VcpuExit::Hlt => {
                vmm_stats.record_hlt();
                println!("\n--- Guest executed HLT ---");
                println!("Guest completed successfully!");
                break;
            }
            VcpuExit::IoOut(port, data) => {
                vmm_stats.record_io_out();
                if port == SERIAL_PORT {
                    for &byte in data {
            ...
            VcpuExit::MmioWrite(addr, data) => {
                ...
                if input_range.contains(&addr) {
                    ...
                    if input_device.should_process_queue() {
                        let io_stats = input_device.process_queue(&guest_mem)?;
                        vmm_stats.record_read(io_stats.bytes_read, io_stats.sectors_read);
                    }
                } else if output_range.contains(&addr) {
                    output_device.mmio_write((addr - OUTPUT_MMIO_BASE) as u32, value);
                    if output_device.should_process_queue() {
                        let io_stats = output_device.process_queue(&guest_mem)?;
                        vmm_stats.record_write(io_stats.bytes_written, io_stats.sectors_written);
                    }
                } else {
            ...
            VcpuExit::FailEntry(reason, cpu) => {
                vmm_stats.record_fail_entry();
                println!("VM Entry Failed! reason=0x{:x}, cpu={}", reason, cpu);
                break;
            }
            exit => {
                vmm_stats.record_unknown();
