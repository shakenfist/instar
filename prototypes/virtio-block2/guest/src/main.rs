...
//! copies all sectors from input to output. Progress and status are reported
//! via structured protobuf messages over the serial port.
...
use serial::{send_capacity, send_complete, send_error, send_init, send_progress};
...
impl VirtioBlock {
    ...
    fn init(mmio_base: usize, vq_base: usize, name: &'static str) -> Option<Self> {
        let dev = Self {
            mmio_base,
            desc_base: vq_base,
...
pub extern "C" fn _start() -> ! {
    ...
    let mut input = match VirtioBlock::init(INPUT_MMIO_BASE, INPUT_VQ_BASE, "input") {
        Some(dev) => dev,
        None => {
    ...
    let mut output = match VirtioBlock::init(OUTPUT_MMIO_BASE, OUTPUT_VQ_BASE, "output") {
        Some(dev) => dev,
        None => {
            send_complete("init", 0, false);
            halt();
    ...
    for sector in 0..total_sectors {
        ...
        if !input.read_sector(sector, &mut buffer) {
            send_error("read", "input", sector, 1);
            errors += 1;
            continue;
        }
        ...
        if !output.write_sector(sector, &buffer) {
            send_error("write", "output", sector, 1);
            errors += 1;
            continue;
        }
        ...
        // Progress every 10 sectors (or on last sector)
        if sector % 10 == 0 || sector == total_sectors - 1 {
            let percent = if total_sectors > 0 {
                ((sector + 1) * 100 / total_sectors) as u32
            } else {
                100
            };
            send_progress("copy", sector + 1, total_sectors, percent);
        }
    }

    send_complete("copy", copied, errors == 0);
    halt();
}
