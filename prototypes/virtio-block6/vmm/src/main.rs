//! Minimal KVM VMM with virtio-block device support and sparse output.
//!
//! This VMM provides two virtio-block devices to the guest:
//! - Input device (read-only): backed by source file
//! - Output device (write): backed by destination file with sparse/dynamic sizing
//!
...
//! a protobuf message.
//!
//! This prototype adds sparse output file support:
//! - Output file is not pre-allocated (grows on demand)
//! - Output capacity can be set independently via --max-output-size
//! - Output file remains sparse if not all sectors are written
//!
//! Includes ioeventfd optimization from virtio-block5 for queue notifications
//! without VM exits.
