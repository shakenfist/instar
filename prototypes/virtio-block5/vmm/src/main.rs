//! Minimal KVM VMM with virtio-block device support and I/O optimizations.
//!
...
//! a protobuf message.
//!
//! This prototype adds I/O path optimizations:
//! - ioeventfd for queue notifications without VM exits
//! - O_DIRECT for direct I/O bypassing the page cache
...
use backing::{BackingMode, BackingStore};
use ioevent::IoEvent;
use stats::VmmStats;
...
#[derive(Parser, Debug)]
#[command(name = "vmm")]
#[command(about = "Virtio-block VMM with I/O optimizations")]
struct Args {
    ...
    /// Use mmap for backing file access
    #[arg(long)]
    mmap_backing: bool,
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Validate backing mode options (mutually exclusive)
    if args.direct_io && args.mmap_backing {
        eprintln!("Error: --direct-io and --mmap-backing are mutually exclusive");
        std::process::exit(1);
    }

    let backing_mode = if args.direct_io {
        BackingMode::Direct
    } else if args.mmap_backing {
        BackingMode::Mmap
    } else {
        BackingMode::Regular
    };
    ...
    // Open backing stores
    let input_backing = BackingStore::open(Path::new(&args.input), true, backing_mode, None)?;
    ...
    println!("Backing mode: {backing_mode}");
