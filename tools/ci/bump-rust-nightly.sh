#!/usr/bin/env bash
#
# Weekly Rust nightly pin bump for the instar devcontainer.
#
# The devcontainer Dockerfile pins its Rust nightly (ARG RUST_NIGHTLY=
# nightly-YYYY-MM-DD) because a broken floating nightly breaks every
# from-scratch image build: on 2026-07-24 the then-current nightly ICE'd
# compiling tokio during `cargo install cargo-audit`, failing CI's
# "Build devcontainer" step. Renovate cannot manage rustup toolchain
# pins (there is no datasource for "latest good nightly"), so this
# script moves the pin forward instead:
#
#   1. Find the newest published nightly (probe the static.rust-lang.org
#      channel manifest, walking back up to a week for skipped days).
#   2. If it is not newer than the current pin, exit — nothing to do.
#   3. Rewrite both Dockerfile pins to the candidate (the dev/test image
#      and the release build image share the pin).
#   4. Validate the candidate END TO END: full builds of BOTH devcontainer
#      images (every cargo tool layer, including the cargo-audit layer
#      that caught the 2026-07-24 ICE), then `make instar` (guest
#      build-std cross-builds), then `make test-rust` (a nightly that
#      miscompiles can build cleanly and still fail tests — see #375),
#      then `make package` and an install of each package on the oldest
#      glibc of its family. A toolchain change reaches the shipped
#      artifact through the packaging metadata as well as through the
#      code, and neither building nor testing can see that: nightly
#      2026-08-17 passed every check above and produced an .rpm
#      requiring libc.so.6(GLIBC_2.18)[WEAK](64bit), which nothing
#      provides, so all three RPM legs of the matrix failed to install
#      (#504). Installing is the only step that catches an
#      unsatisfiable dependency, and the floor check catches a
#      toolchain that raises the binary's glibc requirement.
#   5. Commit the bump, push a bot branch, and open a PR against
#      develop. Any validation failure leaves the pin alone: the job
#      fails, no PR appears, and next week's run tries the then-newest
#      nightly.
#
# A failed docker build never replaces the runner's existing
# `instar-build` tag (docker only retags on success), so a bad candidate
# does not poison other jobs on the same runner.
#
# Usage:
#   tools/ci/bump-rust-nightly.sh [--dry-run]
#
# --dry-run performs discovery, the Dockerfile edit, and the full build
# validation, but skips the commit/push/PR step (for local testing).
# Requires GH_TOKEN (or ambient gh auth) for the PR step.

set -euo pipefail

DRY_RUN=0
if [ "${1:-}" = "--dry-run" ]; then
    DRY_RUN=1
elif [ -n "${1:-}" ]; then
    echo "usage: $0 [--dry-run]" >&2
    exit 2
fi

REPO_ROOT="$(git rev-parse --show-toplevel)"
cd "${REPO_ROOT}"

# Both devcontainer images pin the same nightly and must be bumped and
# validated together: the dev/test image (instar-build) and the release
# build image (instar-release, debian:bullseye).
DOCKERFILE="src/.devcontainer/Dockerfile"
BUILD_DOCKERFILE="src/.devcontainer/build/Dockerfile"
DIST_URL="https://static.rust-lang.org/dist"

# --- 0. Prerequisites -----------------------------------------------------
# Check up front rather than half way through: without this the missing
# tool is only discovered at the validation step, after the Dockerfile has
# already been rewritten, leaving a modified tree behind.
if ! command -v docker >/dev/null 2>&1; then
    echo "ERROR: docker is required to validate a candidate nightly but is not installed." >&2
    echo "       CI installs it in the workflow's 'Install Docker' step." >&2
    exit 1
fi

# --- 1. Current pin -------------------------------------------------------
CURRENT="$(sed -nE 's/^ARG RUST_NIGHTLY=(nightly-[0-9]{4}-[0-9]{2}-[0-9]{2})$/\1/p' "${DOCKERFILE}")"
if [ -z "${CURRENT}" ]; then
    echo "ERROR: no 'ARG RUST_NIGHTLY=nightly-YYYY-MM-DD' pin found in ${DOCKERFILE}" >&2
    exit 1
fi
# The two Dockerfiles must agree; a drift means a previous bump only
# touched one of them. The comparison lives in its own script so that
# pre-commit and build-and-test can run it on every change rather than
# only when this weekly bump fires.
tools/ci/check-devcontainer-pins.sh
echo "Current pin: ${CURRENT}"

# --- 2. Newest published nightly ------------------------------------------
# Nightlies occasionally skip a day, so walk back up to a week from today
# and take the first date whose channel manifest exists.
CANDIDATE=""
for offset in 0 1 2 3 4 5 6; do
    d="$(date -u -d "-${offset} days" +%F)"
    if curl -sfIL -o /dev/null "${DIST_URL}/${d}/channel-rust-nightly.toml"; then
        CANDIDATE="nightly-${d}"
        break
    fi
done
if [ -z "${CANDIDATE}" ]; then
    echo "ERROR: no published nightly found in the last 7 days (network problem?)" >&2
    exit 1
fi
echo "Newest published nightly: ${CANDIDATE}"

# ISO dates compare correctly as strings.
if [ "${CANDIDATE}" \< "${CURRENT}" ] || [ "${CANDIDATE}" = "${CURRENT}" ]; then
    echo "Pin is already current (${CURRENT} >= ${CANDIDATE}); nothing to do."
    exit 0
fi

# Skip if a bump PR for this candidate is already open (e.g. re-run of
# the schedule while last week's PR awaits review).
BRANCH="rust-nightly-bump/${CANDIDATE}"
if [ "${DRY_RUN}" -eq 0 ] && command -v gh >/dev/null 2>&1; then
    if [ -n "$(gh pr list --state open --head "${BRANCH}" --json number --jq '.[].number')" ]; then
        echo "An open PR for ${CANDIDATE} already exists; nothing to do."
        exit 0
    fi
fi

# --- 3. Rewrite the pin ---------------------------------------------------
sed -i -E "s/^ARG RUST_NIGHTLY=nightly-[0-9]{4}-[0-9]{2}-[0-9]{2}$/ARG RUST_NIGHTLY=${CANDIDATE}/" \
    "${DOCKERFILE}" "${BUILD_DOCKERFILE}"
echo "Rewrote ${DOCKERFILE} and ${BUILD_DOCKERFILE} pins to ${CANDIDATE}"

# --- 4. Validate the candidate end to end ---------------------------------
# Force-build BOTH images from their Dockerfiles so the candidate is
# exercised on each (make only rebuilds an image when it is missing, so
# a cached image would otherwise hide a broken nightly).
echo "Building dev/test image (instar-build) with ${CANDIDATE}..."
docker build --pull -t instar-build src/.devcontainer

echo "Building release image (instar-release) with ${CANDIDATE}..."
docker build --pull -t instar-release src/.devcontainer/build

echo "Building instar with ${CANDIDATE}..."
make instar

echo "Running Rust tests with ${CANDIDATE}..."
make test-rust

# Packaging is part of the toolchain's blast radius, and neither the
# build nor the test suite can see it. cargo-generate-rpm derives the
# .rpm's dependencies from the binary's ELF, so a nightly that changes
# how glibc version needs are emitted changes the package without
# changing a line of our code. Only installing it finds that out.
echo "Building packages with ${CANDIDATE}..."
make package

echo "Checking the glibc floor of the ${CANDIDATE} binary..."
tools/ci/check-glibc-floor.sh

echo "Smoke-testing both packages at their glibc floor..."
tools/test-package-install.sh src/target/debian/instar_*.deb ubuntu:22.04
tools/test-package-install.sh src/target/generate-rpm/instar-*.rpm rockylinux:9

echo "${CANDIDATE} validated: both devcontainer image builds, instar build, Rust tests, packaging, glibc floor, and both package installs all pass."

if [ "${DRY_RUN}" -eq 1 ]; then
    echo "--dry-run: skipping commit/push/PR. ${DOCKERFILE} is left modified."
    exit 0
fi

# --- 5. Commit, push, PR --------------------------------------------------
git config user.name "shakenfist-bot"
git config user.email "bot@shakenfist.com"

git checkout -b "${BRANCH}"
git add "${DOCKERFILE}" "${BUILD_DOCKERFILE}"
git commit -m "Bump Rust nightly pin to ${CANDIDATE}.

Weekly automated bump of the devcontainer's pinned Rust nightly
(previously ${CURRENT}), in both the dev/test and release build
Dockerfiles. The candidate was validated on the CI runner before
this PR was opened: both devcontainer image builds (including the
cargo tool layers), \`make instar\`, \`make test-rust\`, \`make
package\`, the glibc floor check, and an install of each package on
the oldest glibc of its family all pass against it.

Generated by tools/ci/bump-rust-nightly.sh via the
rust-nightly-bump workflow."

git push -f -u origin "${BRANCH}"

PR_BODY="Weekly automated bump of the devcontainer's pinned Rust nightly: \`${CURRENT}\` -> \`${CANDIDATE}\`.

Validation performed on the CI runner before opening this PR:

- Full builds of both devcontainer images — dev/test (instar-build) and release (instar-release) — every cargo tool layer, including \`cargo install cargo-audit\` (the layer a broken nightly ICE'd on 2026-07-24)
- \`make instar\` (guest build-std cross-builds on the new nightly)
- \`make test-rust\` (workspace test suite)
- \`make package\` (both the .deb and the .rpm)
- \`tools/ci/check-glibc-floor.sh\` (the binary still honours the published glibc 2.31 promise)
- An install of each package on the oldest glibc of its family — the .deb on \`ubuntu:22.04\`, the .rpm on \`rockylinux:9\`. This is the step that catches a toolchain changing the package's dependencies rather than our code: nightly-2026-08-17 built and tested cleanly and produced an .rpm requiring \`libc.so.6(GLIBC_2.18)[WEAK](64bit)\`, which nothing provides

If this PR looks fine, merge it; if the new nightly is known-bad for reasons the build cannot see, close it and the workflow will propose the next newest nightly on its following run.

---
Generated by \`tools/ci/bump-rust-nightly.sh\` (rust-nightly-bump workflow)."

gh pr create \
    --base develop \
    --assignee mikalstill \
    --reviewer mikalstill \
    --title "Bump Rust nightly pin to ${CANDIDATE}" \
    --body "${PR_BODY}"

echo "Opened bump PR for ${CANDIDATE}."
