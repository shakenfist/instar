...
**Adversarial test images:** Generated by scripts in `imago-testdata/scripts/`
...
Images live in `../imago-testdata/custom/audit/`. Scripts that generate
adversarial or CVE-reproducer images must always be placed in the private
`imago-testdata` repository, never in the public `imago/scripts/` directory.
...
- `scripts/differential-fuzz.py` - Differential fuzzing script (imago vs qemu-img + libyal)
