...
**Adversarial test images:** Generated by scripts in `scripts/` (compression
bombs, circular/deep chains, integer overflow headers, boundary values for
refcount order, virtual size, grain size, dual headers, BAT entries, polyglot
files, truncated headers, VMDK descriptor attacks). Images live in
`../imago-testdata/custom/audit/`. See `PLAN-adversarial-images.md` for
the full adversarial testing strategy.
