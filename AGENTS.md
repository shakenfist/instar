...
**Adversarial test images:** Generated by scripts in
`imago-testdata/scripts/` (compression bombs, circular/deep chains,
integer overflow headers, boundary values, polyglot files, truncated
headers, VMDK descriptor attacks, CVE reproducers). Adversarial
scripts must never be in the public `imago/scripts/` directory.
Images live in `../imago-testdata/custom/audit/`.
